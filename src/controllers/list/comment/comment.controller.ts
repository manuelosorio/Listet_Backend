...
export class CommentController {
  ...
  get = async (req: Request, res: Response): Promise<Query> => {
    return this.db.findListComments(
      req.params.slug,
      async (err: MysqlError, results) => {
        const user: Partial<UserModel> = await this.userService
          .getCurrentUser(req)
          .catch(err => {
            console.log(err);
            return {};
  ...
  post = async (req: Request, res: Response): Promise<any | Response> => {
    ...
    return this.listDb.findListFromID(
      ...
      async (listErr: MysqlError, listResults, _listFields) => {
        ...
        return await this.db.createListComments(
          listComment,
          (commentErr: MysqlError, results, _fields) => {
            if (commentErr) {
