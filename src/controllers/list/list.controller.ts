...
export class ListController {
  ...
  getAll = async (
    ...
    return await this.db.findAllLists(
      async (err, results: ListOwnerModel[]) => {
        if (err) {
  ...
  getSingle = async (
    ...
    return await this.db.findListFromSlug(
      req.params.slug,
      async (err, results) => {
        if (err) {
          console.error(err);
        ...
        if (!results.length)
          return res.status(404).send("List Doesn't Exist.").end();
  ...
  createList = async (
    ...
    return await this.db.createList(list, (err, _results, _fields) => {
      ...
      return res
        .status(201)
        .send({ message: 'List created.', url: list.slug })
        .end();
    });
  };
}
