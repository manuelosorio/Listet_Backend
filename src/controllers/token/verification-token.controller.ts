...
export class VerificationTokenController {
  ...
  verifyAccount = async (req: Request, res: Response): Promise<any> => {
    ...
    await this.db.getVerifyAccountTokenStore(
      tokenStore,
      (err, results: any) => {
        if (err) {
          console.error(err);
        ...
        return !results.length && !!userEmail
          ...
          : this.db.getVerifyAccountTokenStoreExpiration(
              tokenStore,
              (tokenErr, tokenResults) => {
                ...
                return isExpired === true
                  ...
                  : this.db.deleteVerifyTokenStore(
                      tokenStore,
                      (deleteStoreErr, _) => {
                        if (deleteStoreErr) {
                        ...
                        return this.db.userVerify(
                          data,
                          (verifyErr, _verifyResults) => {
                            if (verifyErr) {
