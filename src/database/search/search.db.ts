...
export class SearchDb extends Db {
  ...
  searchUser = async (searchQuery: string, next: queryCallback): Promise<Query> => {
    return this.db.query('SELECT firstName, lastName, username FROM users WHERE firstName LIKE (?) or lastName LIKE (?) next and deactivated=0 ', [`%${searchQuery}%`, `%${searchQuery}%`], next);
  }
}
