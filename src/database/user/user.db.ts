...
export class UserDb extends Db {
   ...
   * @param next
   */
  findUserFromUsername = async (
    username: string,
    next: queryCallback
  ): Promise<Query> => {
    return this.db.query(
      'Select id, email, username, firstName, lastName, verification_status FROM users WHERE username= ?',
      username,
      next
    );
  };
  findUserFromId = async (id: number, next: queryCallback): Promise<Query> => {
    return this.db.query(
      'Select id, email, username, firstName, lastName, verification_status FROM users WHERE id= ?',
      id,
      next
    );
  };
   ...
   * @param next
   */
  findUserFromEmail = async (
    email: string,
    next: queryCallback
  ): Promise<Query> => {
    return this.db.query(
      'Select id, email, username, firstName, lastName, verification_status, deactivated FROM users WHERE email= ?',
      email,
      next
    );
  };
  ...
  newUser = async (user: UserModel, next: queryCallback): Promise<Query> => {
    return this.db.query(
      'INSERT INTO `users` (`firstName`, `lastName`, `email`, `username`, `password`, `admin`, `deactivated`) VALUES (?, ?, ?, ?, ?, ?, ?)',
      [
        user.firstName,
        user.lastName,
        user.email,
