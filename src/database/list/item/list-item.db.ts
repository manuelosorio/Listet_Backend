...
export class ListItemDb extends Db {
  ...
  updateListItemStatus = async (
    ...
    return this.db.query(
      ...
      [listItem.completed, listItem.id],
      next
    );
  };
  updateListItem = async (
    ...
    return this.db.query(
      `UPDATE list_items SET item = ?, deadline = ? WHERE id = ? `,
      [listItem.item, listItem.deadline, listItem.id],
      next
    );
  };
  /**
  ...
  addListItem = async (
    ...
    return this.db.query(
      'INSERT INTO `list_items` (`item`, `deadline`, `completed`, `list_id`) VALUES (?, ?, ?, ?)',
      [listItem.item, listItem.deadline, listItem.completed, listItem.list_id],
      next
    );
  };
}
