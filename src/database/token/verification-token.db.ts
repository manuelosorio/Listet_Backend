...
export class VerificationTokenDb extends Db {
  ...
  verifyAccountToken = async (
    ...
    return this.db.query(
      ...
      [(params as TokenModel).token, (params as UserModel).email],
      next
    );
  };
   ...
   * @param next
   */
  verifyAccountTokenStore = async (
    params: TokenModel,
    next: queryCallback
  ): Promise<Query> => {
    return this.db.query(
      'INSERT INTO `token_verify_account` (token_id, expires, data) VALUES (?, ?, ?)',
      [params.id, params.expires, params.token],
      next
    );
  };
  /**
  ...
  getVerifyAccountTokenStore = async (
    params: string,
    next: queryCallback
  ): Promise<Query> => {
    return this.db.query(
  ...
  getVerifyAccountTokenStoreExpiration = async (
    params: string,
    next: queryCallback
  ): Promise<Query> => {
    return this.db.query(
