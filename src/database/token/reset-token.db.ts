...
export class ResetTokenDb extends Db {
  ...
  resetPasswordToken = async (
    tokenStore: TokenModel,
    next: queryCallback
  ): Promise<Query> => {
    return this.db.query(
      'UPDATE `users` SET reset_token= ? WHERE id= ?',
      [tokenStore.token, tokenStore.id],
      next
    );
  };
  /**
   ...
   * @param next
   */
  resetPasswordTokenStore = async (
    tokenStore: TokenModel,
    next: queryCallback
  ): Promise<Query> => {
    return this.db.query(
      'INSERT INTO `token_reset_password` (token_id, expires, data) VALUES (?, ?, ?)',
      [tokenStore.id, tokenStore.expires, tokenStore.token],
      next
    );
  };
  /**
   ...
   * @param next
   */
  getResetPasswordTokenStoreExpiration = async (
    tokenStore: string,
    next: queryCallback
  ): Promise<Query> => {
    return this.db.query(
  ...
  getResetPasswordTokenStore = async (
    tokenStore: string,
    next: queryCallback
  ): Promise<Query> => {
    return this.db.query(
  ...
  resetPassword = async (
    ...
    return this.db.query(
      `UPDATE users SET reset_token= null, password = ? WHERE email = ?`,
      [params.password, params.email],
      next
    );
  };
  deleteResetTokenStore = async (
    tokenStore: string,
    next: queryCallback
  ): Promise<Query> => {
    return this.db.query(
      'DELETE FROM token_reset_password WHERE token_id= ?',
      [tokenStore],
      next
    );
  };
