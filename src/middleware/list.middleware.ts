...
export function isItemEmpty(
  ...
  if (req.body.item.length < 1)
    return res.status(403).send({ message: "Item Can't be empty." });
  return next();
}
...
export const isListOwner = async (
  ...
  return res
    .status(400)
    .send({ message: "You don't have permission to complete that action." })
    .end();
};
export const isListItemOwner = async (
  req: Request,
  res: Response,
  next: NextFunction
): Promise<void | Response> => {
...
export const doesListExist = async (
  req: Request,
  res: Response,
  next: NextFunction
): Promise<void | Response> => {
  listService.testSlug(req.body.slug).then(r => {
    if (r) {
      return res
        .status(409)
        .send({ message: 'A list by that name already exists.' })
        .end();
    }
    return next();
  });
...
export const isListPrivate = async (
  req: Request,
  res: Response,
  next: NextFunction
): Promise<void | Response> => {
  return listService
    .checkVisibilityStatus(req.params.slug)
    .then(async results => {
      if (Number(results.visibility) == ListVisibility.private) {
        if (req.session.user) {
          const isOwner = await listService.isListOwner(
            req.session.user.id as unknown as number,
            results.id as unknown as number
          );
          ...
          return isOwner
            ...
            : res
                .status(403)
                .send({ message: 'You must own this list to view it' })
                .end();
        }
        return res
          .status(403)
